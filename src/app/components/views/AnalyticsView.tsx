...
export function AnalyticsView() {
  ...
  const topPosts = mockAnalytics.topPosts.map(post => ({
    _id: post.id,
    author: {
      _id: post.author.id,
    ...
    <div className="mx-auto max-w-6xl pb-20">
      ...
      <div className="space-y-6 p-4">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card className="border-border shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-bold uppercase tracking-tight opacity-70">Total Posts</CardTitle>
          ...
          <Card className="border-border shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-bold uppercase tracking-tight opacity-70">Total Likes</CardTitle>
          ...
          <Card className="border-border shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-bold uppercase tracking-tight opacity-70">Comments</CardTitle>
          ...
          <Card className="border-border shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-bold uppercase tracking-tight opacity-70">Followers</CardTitle>
            ...
            <CardContent>
              ...
              <p className="flex items-center gap-1 text-xs text-green-500 font-bold">
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Legend
                  ...
                  iconType="circle"
                  formatter={(value) => (
                    <span className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mx-2">
                      {value}
                    </span>
                  )}
                />
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
           ...
           <Card className="border-border shadow-sm">
            <CardHeader>
              <CardTitle className="tracking-tight text-lg">Audience Growth</CardTitle>
              <CardDescription className="font-medium opacity-70">
            ...
            <CardContent className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={mockAnalytics.audienceGrowth}>
                  ...
                  <YAxis stroke="hsl(var(--muted-foreground))" fontSize={11} fontWeight={600} hide />
                  <Tooltip />
                  <Area
                    type="monotone"
                    dataKey="followers"
          ...
          <Card className="border-primary/20 bg-primary/5 shadow-inner">
            ...
            <CardContent className="space-y-4">
               ...
               {[
                 { title: "Peak Engagement", desc: "Your audience is most active on Wednesdays at 2 PM." },
                 { title: "Visual Advantage", desc: "Posts with media receive 40% more engagement." },
                 { title: "Topic Trend", desc: "Try posting about #AIRevolution or #WebDev." }
               ].map((insight, i) => (
                 <div key={i} className="bg-background/50 rounded-xl p-3 border border-border/50">
                    <h4 className="text-xs font-bold uppercase tracking-widest text-primary mb-1">{insight.title}</h4>
                    <p className="text-xs font-medium text-muted-foreground leading-snug">{insight.desc}</p>
                 </div>
               ))}
            </CardContent>
          </Card>
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-bold tracking-tight px-1">Top Performing Content</h3>
          <div className="grid grid-cols-1 gap-4">
            {topPosts.map(post => (
              <div
                key={post._id}
                className="border-border overflow-hidden rounded-2xl border bg-card shadow-sm hover:shadow-md transition-shadow"
              >
                <PostCard 
                  post={post} 
